...
BOOL CJoyPage::OnInitDialog()
{
	...
	for (i=0; i<2; i++) {
		...
		if (m_pConfig->joy_dev[i] < pComboBox->GetCount()) {
			...
			// 設定値が、存在するデバイス数を超えている→No Assignにフォーカス
			pComboBox->SetCurSel(0);
		}
...
{
	// CInputに対して独自にApplyCfg(設定を編集前に戻す)
...
BOOL CBtnSetPage::OnInitDialog()
{
	...
	// 親クラスの初期化(CPropertySheetはOnInitDialogを持たない)
