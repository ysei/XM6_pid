...
#ifdef OMF
	emit("section _DATA use32 class=DATA\n");
#else
	emit("section .data align=16\n");
#endif
	emit("bits 32\n");
...
/*	align(4);*/
	...
	*/
...
#ifdef XM6
	// __io_cycle_counterを外部に公開する(XM6拡張)
	emit("GLOBAL _%siocycle\n", sourcename);
	emit("_%siocycle:\n", sourcename);
#endif
	...
	// __access_addressが0x00000000なら、バスエラーとみなす(XM6拡張)
	emit("__access_address       dd 0\n");

emit("save_01                dd 0\n");		// Stef Fix( Gens)

#ifdef XM6
	emit("__FETCH_REGION_OFFSET  DD 0\n");
	emit("__BACKUP_ESP           DD 0\n");
	emit("__BUSERR_ADDR          DD 0\n");
	emit("__BUSERR_STAT          DD 0\n");
#endif
}
...
static void gen_interface(void) {
#ifdef OMF
	emit("section _TEXT use32 class=CODE\n");
#else
	emit("section .text align=16\n");
#endif
	emit("bits 32\n");
	emit("top:\n");

#ifdef XM6
	emit("EXTERN _s68000intack\n");
	emit("EXTERN _s68000buserrlog\n");
	emit("EXTERN _s68000addrerrlog\n");
	emit("GLOBAL _s68000wait\n");
	emit("_s68000wait:\n");
	emit("MOV EAX,-1\n");
	emit("CMP EAX,[__io_cycle_counter]\n");
	emit("JZ .ret\n");
	emit("MOV EAX,[ESP+4]\n");
	...
	emit("INC EAX\n");
	emit("RET\n");
	emit("GLOBAL _s68000getcounter\n");
	emit("_s68000getcounter:\n");
	emit("MOV EAX,[__io_cycle_counter]\n");
	emit("RET\n");

	emit("GLOBAL _s68000buserr\n");
	emit("_s68000buserr:\n");
	emit("PUSH EBP\n");
	emit("MOV EBP,ESP\n");
	emit("MOV EAX,[ESP+12]\n");
	emit("; upper - instruction, lower - function & bits (not 0)\n");
	emit("MOV [__BUSERR_STAT],EAX\n");
	emit("MOV EAX,[ESP+8]\n");
	emit("MOV [__BUSERR_ADDR],EAX\n");
	emit("XOR EAX,EAX\n");
	emit("MOV [__access_address],EAX\n");
	emit("POP EBP\n");
	emit("RET\n");

	emit("addr_error:\n");
	emit("PUSHAD\n");
	emit("MOV EAX,[__BUSERR_STAT]\n");
	emit("AND EAX,00007FFFh\n");
	emit("MOV [__BUSERR_STAT],EAX\n");
	emit("PUSH EAX\n");
	emit("MOV EAX,[__BUSERR_ADDR]\n");
	emit("PUSH EAX\n");
	emit("CALL _s68000addrerrlog\n");
	emit("ADD ESP,8\n");
	emit("POPAD\n");
	emit("MOV EDX,0CH\n");
	emit("JMP short bus_error0\n");

	emit("bus_error:\n");
	emit("TEST dword[__BUSERR_STAT],00008000h\n");
	emit("JNZ addr_error\n");
	emit("PUSHAD\n");
	emit("MOV EAX,[__BUSERR_STAT]\n");
	emit("PUSH EAX\n");
	emit("MOV EAX,[__BUSERR_ADDR]\n");
	emit("PUSH EAX\n");
	emit("CALL _s68000buserrlog\n");
	emit("ADD ESP,8\n");
	emit("POPAD\n");
	emit("MOV EDX,08H\n");

	emit("bus_error0:\n");
	emit("MOV ESP,[__BACKUP_ESP]\n");
	emit("CALL group_1_exception\n");
	emit("PUSH ESI\n");
	emit("MOV EDX,[__a7]\n");
	emit("SUB EDX,BYTE 2\n");
	emit("MOV ECX,[__BUSERR_STAT]\n");
	emit("ROR ECX,16\n");
	emit("CALL writememoryword\n");
	emit("SUB EDX,BYTE 4\n");
	emit("MOV ECX,[__BUSERR_ADDR]\n");
	...
	emit("MOV [__a7],EDX\n");
	emit("XOR ESI,ESI\n");
	emit("MOV [__BUSERR_STAT],ESI\n");
	emit("POP ESI\n");
// 2005.1.30 ＰＩ．
// group_1_exception, group_2_exceptionで常にfetch_regionを考慮するため削除
#if 0
	emit("CMP ESI,[__fetch_region_start]\n");
	emit("JB SHORT bus_error1\n");
	emit("CMP ESI,[__fetch_region_end]\n");
	emit("JBE SHORT bus_error2\n");
	emit("bus_error1:\n");
	emit("CALL basefunction\n");
	emit("bus_error2:\n");
	emit("ADD ESI,EBP\n");
#endif
	emit("SUB EDI,BYTE 50\n");
	emit("JMP execend\n");
#endif
...
*/
	...
	*/
	...
	// 2005.1.30 ＰＩ．
	// group_1_exceptionでre-baseを行うため、辻褄あわせ
	emit("SUB ESI,EBP\n");
...
*/
	...
	emit("RET\n");
	emit(".live:\n");
	emit("SUB EAX,[__FETCH_REGION_OFFSET]\n");
	maskaddress("EAX");
	emit("RET\n");
#else
	emit("test byte[__execinfo],1\n");
...
static void gen_basefunction(void) {
...
#ifdef XM6
	emit("MOV [__BUSERR_ADDR],esi\n");
	emit("MOV [__BUSERR_STAT],dword 001eh\n");
	emit("JMP bus_error\n");
#else
	...
	*/
	...
	emit("mov ebp,[edx+8]\n");
#ifdef XM6
	emit("MOV [__FETCH_REGION_OFFSET],EBP\n");
#endif
	emit("pop edx\n");
	if(addressbits < 32) {
		/*
...
static void gen_readbw(int size){
	...
	emit("ADD EDX,EDX\n");
	emit("INC EDX\n");
	emit("MOV [__BUSERR_ADDR],EDX\n");
	emit("MOV [__BUSERR_STAT],word 001dh\n");
	emit("JMP addr_error\n");
	}
#endif
	...
	*/
#ifdef XM6
	// ReadByte, ReadWordの呼び出し規約:
	// EDX=ADDR (上位8bitの0を保証)
	// EAX=RET (上位にゴミがあってもよい)
	// アドレスエラー又はバスエラーは、__access_addressに0を入れる
...
static void gen_writebw(int size){
	...
	*/
	airlock_exit();
#ifdef XM6
	// WriteByte, WriteWordの呼び出し規約:
	// EDX=ADDR (上位8bitの0を保証)
	// EBX=DAT (上位の0を保証))
	// アドレスエラー又はバスエラーは、__access_addressに0を入れる
	emit("XOR EBX,EBX\n");
	emit("MOV %s,[ESP+%d]\n",x86bx[size],airlock_stacksize);
	emit("CALL dword[ECX+8]\n");
#else
	emit("mov ebx,ecx\n");      /* pointer to structure */
	emit("mov eax,edx\n");      /* address */
...
static void gen_group_12_exception(void) {
	...
	// 2005.1.30 ＰＩ．
	// ユーザモードで各種割り込みがかかると、ベクタエリアが読み取れず
	// ダブルフォールトになる問題の対策．
#if 1
	// 68000のみ対応
	if (cputype >= 68010) {
...
static void gen_privilege_violation(void){
	...
	emit("mov edx,20h\n");
	emit("call group_1_exception\n");
//	perform_cached_rebase();			// 2005.1.30 ＰＩ．
...
static void i_jmp(void){
	...
	emit("INC ESI\n");
	emit("PUSH ESI\n");
	emit("SUB ESI,[__FETCH_REGION_OFFSET]\n");
	maskaddress("ESI");
	emit("MOV [__BUSERR_ADDR],ESI\n");
	emit("POP ESI\n");
	emit("MOV [__pc],ESI\n");
	emit("MOV [__BUSERR_STAT],word 0016h\n");
	emit("JMP addr_error\n");
#endif
}
...
static void i_rte(void){
	...
	if(cputype>=68010){
		...
		** that's not implemented yet). */
		...
		// 2005.1.30 ＰＩ．
		// 最後のA7からの戻りPC読み取りが、ユーザモードで行われる可能性あり
#if 1
		emit("ADD EDX,BYTE 2\n");
		emit("CALL readmemory%s\n",sizename[4]);
		emit("PUSH ECX\n");
...
#ifdef OMF
	emit("section _BSS use32 class=BSS\n");
#else
	emit("section .bss align=16\n");
#endif
	emit("bits 32\n");
	align(4);
